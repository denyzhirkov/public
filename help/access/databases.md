# Databases
 ...
 Datagrok lets you connect databases and work with them via **Database
 Manager**. To open **Database Manager**, from the left side panel, click
 **Data** > **Databases**.

In this article:

* [Accessing databases](databases.md/#accessing-databases)
...
> The platform provides the creation of a _database connection_ to all the
> popular databases, including PostgreSQL, MySQL, MS SQL, Maria DB, Oracle, and
> others.
>
...
complete list of supported connectors and required parameters, see [Supported
...
1. From the left side panel, click **Data** > **Databases**.
1. Right-click the appropriate connector and choose **Add connection…**. This
   action opens the **Add new connection** dialog.
1. Fill in the dialog fields displayed.
   >Notes:
   >
   > If the connection supports a custom JDBC connection string, you have an
   > option to fill in the parameter **Conn. string** instead of other
...
You can modify a database connection at any time:
...
* [Query a database](databases.md/#query-a-database):
  * Write a SQL query
  * Create a visual aggregation query
...
1. From the left side panel, click **Data** > **Databases**. This action shows a
   list of available connectors.
1. To see the available connections list, expand the connector.
1. Right-click the database connection and click **Add query…**. The **Query
   View** opens.
1. In the **Query View**, write the SQL query.
   > Note: At any stage of writing a query, you can run it and preview its
   > results. To run a query, press **F5** or click the **Run query** icon on
   > the **Menu Ribbon**. Query results appear at the bottom of the **Query
   > View** window.
1. When finished, enter a query name, then on the **Menu Ribbon**, click
   **Save**.

![Add query](../uploads/gifs/query-add.gif "Add query")

To see a complete list of available options for **querying a specific table**,
follow these steps:

1. From the left side panel, click **Data** > **Databases**. This action shows a
   list of available connectors.
1. Expand the connector and then the database connection from the available
   connections list.
1. Depending on the connector, to access the database’s tables,  expand
   **Schemas** or **Schemas** > **public**.
...
* [Aggregate data via visual
  query](databases.md/#create-a-visual-aggregation-query) (**Visual Query…**).
* [Join tables](databases.md/#join-tables) (**Build Query…**).

#### Write a SQL query

To write a SQL query, follow these steps:
...
For some [data sources](connectors/supported-connectors.md), Datagrok detects
the database's schema. When you open a table in **Query builder**, the platform
automatically displays all the tables connected to it by the foreign keys.
...
1. On the right side of the dialog, select required columns from tables that the
   platform automatically presents for joining. The platform immediately
   generates a SQL query on the left side of the dialog, which you can edit.
   Datagrok instantaneously shows the results of the query in the dataframe
   below.
...
To parameterize a query, annotate input or output parameters in the query’s
header using SQL/Sparql comments: `--` for SQL and `#` for Sparql.

For example, the following  SQL query defines a `freight` input parameter:
